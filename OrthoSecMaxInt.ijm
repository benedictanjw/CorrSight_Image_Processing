...
run("RGB Color");
run("Scale Bar...", "width=10 height=4 font=14 color=White background=Black location=[Lower Right] bold hide overlay");
run("Flatten"); // Burn crosshairs and scalebar into image
saveAs("Tiff", fdir+fname3+"_MI"); // Save
close();
selectWindow("MAX_"+fname+" (RGB)");
close();
selectWindow("MAX_"+fname);
close();
...
run("RGB Color", "keep");
run("Scale Bar...", "width=10 height=4 font=14 color=White background=Black location=[Lower Right] bold hide overlay");
run("Flatten"); // Burn crosshairs and scalebar into copy of image
saveAs("Tiff", fdir+fname3); // Save
close();
selectWindow(fname2+"-1.tif");
close();
}
}
